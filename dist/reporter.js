...
function groupByScanner(findings) {
    const map = new Map();
    for (const f of findings) {
        if (!map.has(f.scanner))
            map.set(f.scanner, []);
...
function generateSummary(findings) {
    ...
    const parts = [];
    if (critCount > 0) {
        parts.push(`This skill has ${critCount} CRITICAL issue(s) that pose serious security risks.`);
    }
    if (highCount > 0) {
        parts.push(`There are ${highCount} HIGH severity issue(s) that warrant careful review.`);
    }
